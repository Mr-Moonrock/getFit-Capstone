...
describe('Abs Component with ExpandableCard', () => {
  beforeEach(() => {
    mockAxios.reset();
  ...
  test('fetches and displays exercises in ExpandableCard', async () => {
    ...
    mockAxios.onGet('https://exercisedb.p.rapidapi.com/exercises/target/abs')
  ...
  test('renders exercises correctly and handles "Next" and "Prev" button click', async () => {
    ...
    const nextButton = screen.getByTestId('abs-next-btn');
    ...
    const prevButton = screen.getByTestId('abs-previous-btn');
  ...
  test('displays an error message when data fetch fails', async () => {
    ...
    await waitFor(() => {
      const errorMessage = screen.getByTestId('abs-error-message');
