...
describe('Cardio Component with ExpandableCard', () => {
  beforeEach(() => {
    mockAxios.reset();
  ...
  test('fetches and displays exercises in ExpandableCard', async () => {
    ...
    const target = 'cardiovascular system';
  ...
  test('disables "Prev" button on the first page and enables it on subsequent pages', async () => {
    ...
    const prevButton = screen.getByTestId('cardio-previous-btn');
    expect(prevButton).toBeDisabled();

    const nextButton = screen.getByTestId('cardio-next-btn');
  ...
  test('displays an error message when data fetch fails', async () => { 
    ...
    await waitFor(() => {
      const errorMessage = screen.getByTestId('cardio-error-message');
