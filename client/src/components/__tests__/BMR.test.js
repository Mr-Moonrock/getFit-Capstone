...
describe('BMR Component', () => {
  ...
  it('should redirect to /login if no user is present', async () => {
    render(
      <MemoryRouter initialEntries={['/bmr']}>
  ...
  it('should display BMR calculation from fetched data', async () => {
    const mockUser = { id: 1, username: 'testuser' };
    const userBmrData = {
      userBmrData: { bmr: 2500 }
    };

    global.fetch = jest.fn(() =>
      Promise.resolve({
        json: () => Promise.resolve(userBmrData)
      })
    );
  ...
  it('should display error message if data fetching fails', async () => {
    ...
    await waitFor(() => {
      expect(console.error).toHaveBeenCalledWith('Error getting bmr values');
    });
