...
function Back () {
  const targets = [ 'pectorals', 'serratus anterior']
  ...
  const [activeTab, setActiveTab] = useState('pectorals');
  ...
  return (
  <div className='container' id='chest-container'>
    <div className="card text-center">
      <div className="card-header">
        <ul className="nav nav-tabs card-header-tabs" id='chest-nav-tabs'>
          {targets.map(target => (
          <li className="nav-item" id='chest-nav-item' key={target}>
      ...
      <div className="card-body">
        {error ? (
          <div className="alert alert-danger" data-testid="chest-error-message">
          {error}
        </div>
        ) : (
          activeTab && fetchedExercisesByTarget[activeTab] && (
            <div className='chest-card-container'>
              <div className="card-grid-container row">
                ...
                <div key={index} className='chest-card col-6'>
      ...
      <div className="card-footer">
        <button className='chest-prev' 
                data-testid='chest-previous-btn'
                onClick={() => handlePrev(activeTab)}
                disabled={currentIndexByTarget[activeTab]?.currentIndex === 0}
        > Prev 
        </button>
        <button className='chest-next' 
                data-testid='chest-next-btn'
