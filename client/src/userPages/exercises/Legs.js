...
function Legs() {
  ...
  return (
  <div className='container' id='legs-container'>
    <div className="card text-center">
      <div className="card-header">
        <ul className="nav nav-tabs card-header-tabs" id='legs-nav-tabs'>
          {targets.map(target => (
          <li className="nav-item" id='legs-nav-item' key={target}>
      ...
      <div className="card-body">
        {error ? (
          <div className="alert alert-danger" data-testid="legs-error-message">
          {error}
        </div>
        ) : (
          activeTab && fetchedExercisesByTarget[activeTab] && (
           <div className='legs-card-container'>
              <div className="card-grid-container row">
                ...
                <div key={index} className='legs-card col-6'>
      ...
      <div className="card-footer">
        <button className='legs-prev' 
                data-testid='legs-previous-btn'
                onClick={() => handlePrev(activeTab)}
                disabled={currentIndexByTarget[activeTab]?.currentIndex === 0}
        > Prev 
        </button>
        <button className='legs-next' 
                data-testid='legs-next-btn'
