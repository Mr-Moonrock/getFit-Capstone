...
import './styles/Arms.css';

function Arms () {
  const targets = [ 'triceps', 'biceps' ];
  ...
  const [activeTab, setActiveTab] = useState('triceps');
  ...
  return (
  <div className='container' id='arms-container'>
    <div className="card text-center">
      <div className="card-header">
        <ul className="nav nav-tabs card-header-tabs" id='arms-nav-tabs'>
          {targets.map(target => (
       ...
       <div className="card-body">
          {error ? (
            <div className="alert alert-danger" data-testid="arms-error-message">
            {error}
          </div>
          ) : (
            activeTab && fetchedExercisesByTarget[activeTab] && (
              <div className='arms-card-container'>
                <div className="card-grid-container row">
                  ...
                  <div key={index} className='arms-card col-6'>
